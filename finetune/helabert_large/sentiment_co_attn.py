...
# ==================== CONFIGURATION ====================
print("=" * 80)
print("HelaBERT FINE-TUNING — SENTIMENT  [CO-ATTENTION FIXED | 5 SEED RUNS]")
print("=" * 80)
...
# ==================== CO-ATTENTION MODULE (FIXED) ====================
class CoAttention(nn.Module):
    """
    Parallel co-attention between [CLS] and the token sequence.

    Given:
        cls_vec  : (B, H)    — the [CLS] token
        token_seq: (B, T, H) — the remaining token sequence
        key_mask : (B, T)    — 1=real token, 0=padding

    Shared affinity (B, T):
        affinity = v( tanh( W_cls([CLS]).expand + W_token(token_seq) ) )

    Direction 1 — [CLS] attends over tokens  → attended_cls    (B, H)
        alpha    = softmax_safe(affinity)
        attended_cls = alpha @ token_seq

    Direction 2 — tokens attend over [CLS]   → attended_tokens (B, H)
        Each token's weight towards [CLS] is proportional to its affinity
        score (high affinity = more [CLS]-like). We use a sigmoid gate
        per token and take a weighted sum of the tokens themselves,
        producing a "[CLS]-guided" summary of the sequence.

        beta             = sigmoid(affinity) * token_mask        (B, T)
        beta_norm        = beta / (beta.sum(-1, keepdim=True) + ε)
        attended_tokens  = beta_norm @ token_seq                 (B, H)

    FIX 1: softmax_safe clamps all-padding rows to uniform distribution
            instead of producing NaN.
    FIX 2: Direction 2 uses sigmoid + renorm instead of the broken
            transpose(0,1) which was swapping batch and time axes.
    """
    ...
    def softmax_safe(logits, mask=None):
        """
        Softmax that is safe even when an entire row is masked (-inf).
        Masked positions are set to -1e9 (not -inf) so that if all
        positions in a row are padding, softmax returns a uniform
        distribution instead of NaN.
        """
        if mask is not None:
            # mask: 1=keep, 0=pad  (-1e4 is fp16-safe; -1e9 overflows float16)
    ...
    def forward(self, cls_vec, token_seq, key_mask=None):
        ...
        cls_vec  : (B, H)
        token_seq: (B, T, H)
        key_mask : (B, T)  — 1=real, 0=pad
        """
        B, T, H = token_seq.shape

        # ── Shared affinity ────────────────────────────────────────────────
        cls_proj   = self.W_cls(cls_vec).unsqueeze(1).expand(-1, T, -1)  # (B, T, H)
        token_proj = self.W_token(token_seq)                              # (B, T, H)
        affinity   = self.v(torch.tanh(cls_proj + token_proj)).squeeze(-1) / self.scale
        # affinity: (B, T)

        # ── Direction 1: [CLS] attends over token_seq ─────────────────────
        # NaN-safe: use -1e9 instead of -inf for masked positions
        alpha        = self.softmax_safe(affinity, key_mask)              # (B, T)
        attended_cls = torch.bmm(alpha.unsqueeze(1), token_seq).squeeze(1) # (B, H)

        # ── Direction 2: tokens attend over [CLS] ─────────────────────────
        # Each token's relevance to [CLS] is its affinity score.
        # We gate by the real-token mask, renormalize, then take the
        # weighted sum of token representations — giving a [CLS]-guided
        # summary of the actual content tokens.
        beta = torch.sigmoid(affinity)                                    # (B, T)
        if key_mask is not None:
            beta = beta * key_mask.float()                                # zero out pads
        beta_norm       = beta / (beta.sum(dim=-1, keepdim=True) + 1e-9) # (B, T)
        attended_tokens = torch.bmm(beta_norm.unsqueeze(1), token_seq).squeeze(1)  # (B, H)

        return attended_cls, attended_tokens


# ==================== MODEL ====================
class SentimentModelCoAttention(nn.Module):
    """
    ...
    Pipeline:
        BERT encoder
          ...
          token_mask(B, T)    = attention_mask[:, 1:]

        CoAttention(cls_vec, token_seq, token_mask)
          → attended_cls    (B, H)
          → attended_tokens (B, H)
    ...
    def __init__(self, bert, hidden_size, num_labels, dropout=0.1):
        ...
        # LayerNorm on attended outputs before concat — stabilises early training
    ...
    def forward(self, input_ids, attention_mask, labels=None):
        ...
        hidden = out.last_hidden_state                  # (B, T_full, H)

        cls_vec    = hidden[:, 0, :]                    # (B, H)
        token_seq  = hidden[:, 1:, :]                   # (B, T, H)
        token_mask = attention_mask[:, 1:]              # (B, T) — 1=real, 0=pad
...
def load_fresh_model(num_labels):
    ...
    return SentimentModelCoAttention(backbone, cfg.hidden_size, num_labels, DROPOUT)
...
pd.DataFrame({'id': range(NUM_LABELS), 'label': le.classes_}).to_csv(
...
for run_idx, seed in enumerate(SEEDS, start=1):
    ...
    train_ds = SentimentDataset(train_texts, train_labels, sp, MAX_SEQ_LENGTH)
