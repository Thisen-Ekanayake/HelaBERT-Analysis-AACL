...
# HelaBERT max position embeddings is 512; truncate to 512.
MAX_SEQ_LENGTH = 512    # truncation applied — paper explicitly uses this for writing style
BATCH_SIZE     = 16
LEARNING_RATE  = 1e-5
NUM_EPOCHS     = 3
DROPOUT        = 0.1
NUM_LABELS     = 4      # News, Academic, Blog, Creative
...
print(f"  Long docs: truncated to {MAX_SEQ_LENGTH} tokens (paper method)")
...
class WritingStyleDataset(Dataset):
    """
    Long documents are TRUNCATED to MAX_SEQ_LENGTH tokens.
    """
    ...
    def __getitem__(self, idx):
        # Truncate long documents to max_length (paper method)
...
def load_fresh_model(num_labels):
    ...
    return WritingStyleModel(backbone, cfg.hidden_size, num_labels, DROPOUT)
...
# Paper pre-processing: remove English-only text and very long text (> 3500 chars)
...
if num_labels_actual != NUM_LABELS:
    print(f"WARNING: Expected {NUM_LABELS} classes, found {num_labels_actual}: {list(le.classes_)}")
    NUM_LABELS = num_labels_actual
...
# Log average token length (to confirm truncation impact)
sample_lengths = [len(sp.encode(t)) for t in all_texts[:500]]
print(f"\nAvg token length (sample of 500): {np.mean(sample_lengths):.1f} "
      f"(truncated at {MAX_SEQ_LENGTH})")
...
print(f"Note: Long documents truncated to {MAX_SEQ_LENGTH} tokens (paper method)")
print("=" * 80)
...
for run_idx, seed in enumerate(SEEDS, start=1):
    ...
    train_ds = WritingStyleDataset(train_texts, train_labels, sp, MAX_SEQ_LENGTH)
    test_ds  = WritingStyleDataset(test_texts,  test_labels,  sp, MAX_SEQ_LENGTH)
    ...
    if USE_WANDB:
        ...
        wandb.init(
            ...
            config={
                "task": "writing_style", "method": "paper_sinbert",
                ...
                "truncation": True, "test_size": TEST_SIZE,
            },
            reinit=True,
        )
