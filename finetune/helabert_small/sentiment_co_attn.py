...
# ==================== CONFIGURATION ====================
print("=" * 80)
print("HelaBERT FINE-TUNING — SENTIMENT  [CO-ATTENTION | 5 SEED RUNS]")
print("=" * 80)
...
OUTPUT_DIR    = "HelaBERT_coattention_sentiment"
USE_WANDB     = True
WANDB_PROJECT = "helabert-coattention"
WANDB_GROUP   = "sentiment_small_5runs_coattn"
...
class CoAttention(nn.Module):
    ...
    def softmax_safe(logits, mask=None):
        # fp16-safe fill: -1e4 avoids overflow while preventing NaN on all-pad rows
        if mask is not None:
...
class SentimentModelCoAttention(nn.Module):
    """
    Co-Attention classifier on top of HelaBERT (small).

    Pipeline:
        BERT encoder → cls_vec + token_seq
        CoAttention → attended_cls + attended_tokens
        LayerNorm → concat → (B, 2H)
...
for run_idx, seed in enumerate(SEEDS, start=1):
    ...
    if USE_WANDB:
        ...
        wandb.init(
            ...
            config={
                "task": "sentiment_analysis", "method": "co_attention",
                ...
                "head": "CoAttention -> LayerNorm -> concat -> MLP",
            },
            reinit=True,
        )
